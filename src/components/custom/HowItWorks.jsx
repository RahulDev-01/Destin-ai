    ...
    <section className='relative w-full py-24 md:py-32 bg-white overflow-hidden'>
      ...
      <div className='relative mx-auto max-w-7xl px-4 sm:px-8 lg:px-10'>
        ...
        <div className='relative'>
          {/* Connecting Line */}
          <div className='hidden lg:block absolute top-1/2 left-0 right-0 h-1 bg-blue-300 transform -translate-y-1/2 z-0'></div>
            ...
            {steps.map((step, index) => (
              <div key={index} className='relative group'>
                ...
                <div className='bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-lg hover:shadow-xl transition-shadow duration-300 border border-gray-100 h-full'>
                  {/* Step Number */}
                  <div className={`absolute -top-4 -right-4 sm:-top-6 sm:-right-6 w-12 h-12 sm:w-16 sm:h-16 bg-black rounded-2xl flex items-center justify-center text-white font-black text-lg sm:text-xl shadow-xl`}>
                    {step.number}
                  </div>

                  {/* Icon */}
                  <div className={`inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 ${step.bgColor} rounded-2xl mb-6 text-4xl sm:text-5xl font-black text-blue-700 shadow-md`}>
                    {index + 1}
                  </div>
